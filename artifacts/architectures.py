"""
Самодостаточные классы нейросетевых архитектур.

Каждый класс полностью определён здесь (без замыканий на внешние объекты,
без лямбд, без anonymous nn.Sequential на верхнем уровне). Это позволяет
сохранять артефакт как (state_dict, config) и потом восстанавливать модель
в любом окружении, где доступен этот файл.
...
Конвенция: для тренировки и инференса модель принимает float32-тензор
